...
;(function (name, definition) {
  ...
  EventProxy.prototype.order = function (eventName, total, callback, done) {
    var group = eventName + '_group';
    this._after = this._after || {};
    this._after[group] = {
      index: 0
    ...
    var fire = function (i) {
      ...
      callback(results[i]);
      if (i === total - 1) {
        proxy.unbind(eventName, handler);
        proxy.unbind(group, groupHandler);
        done && done(results);
      } else if (received[i + 1]) {
        fire(i + 1);
      }
    };
    var groupHandler = function (e) {
      handler(e.index, e.result);
    };
    proxy.on(eventName, handler);
    proxy.on(group, groupHandler);
  };
